...
Revision ID: mom_09_seed_data
Revises: mom_08_procedures

Auto-generated from the Momentra SQL design documents. The SQL is preserved
verbatim in the sibling ``sql/mom_09_seed_data.up.sql`` / ``sql/mom_09_seed_data.down.sql`` files
...
revision = "mom_09_seed_data"
down_revision = "mom_08_procedures"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    _run("mom_09_seed_data.up.sql", record=True)

    bind = op.get_bind()
    rows = list(bind.exec_driver_sql("SELECT id, sql FROM mom_migration_skips ORDER BY id"))
    for row in rows:
        sp = bind.begin_nested()
        try:
            bind.exec_driver_sql(row[1])
            sp.commit()
            d = bind.begin_nested()
            try:
                bind.exec_driver_sql("DELETE FROM mom_migration_skips WHERE id = %d" % row[0])
                d.commit()
            except Exception:
                d.rollback()
        except Exception:
            sp.rollback()
    remaining = list(bind.exec_driver_sql("SELECT count(*) FROM mom_migration_skips"))[0][0]
    if remaining:
        _log.warning(
            "Momentra migrations complete: %s source statement(s) could not be "
            "applied and are logged in table mom_migration_skips.", remaining,
        )


def downgrade() -> None:
    _run("mom_09_seed_data.down.sql", record=False)
