...
class RunwayTemplateBuilder:
    ...
    async def build(self, user_id: UUID, moment_id: UUID) -> RunwayContext:
        ...
        inflow_sum = await self.session.execute(
            select(func.coalesce(func.sum(RunwayCashInflows.amount_in_operating_currency), 0))
            .where(RunwayCashInflows.moment_id == moment_id)
        )
        total_inflow = int((inflow_sum.scalar() or 0) * 100)

        burn_sum = await self.session.execute(
            select(func.coalesce(func.sum(RunwayExpenseBurns.amount_in_operating_currency), 0))
            .where(RunwayExpenseBurns.moment_id == moment_id)
        )
        total_burn = int((burn_sum.scalar() or 0) * 100)

        risk_q = await self.session.execute(
            select(func.count()).select_from(RunwayRisks).where(
        ...
        dec_q = await self.session.execute(
            select(func.count()).select_from(RunwayStrategicDecisions).where(
                RunwayStrategicDecisions.moment_id == moment_id,
                RunwayStrategicDecisions.decision_status == "active",
            )
        )
        decision_count = dec_q.scalar() or 0

        fu_q = await self.session.execute(
            select(func.count()).select_from(RunwayFinancialUpdates).where(
                RunwayFinancialUpdates.moment_id == moment_id,
            )
        )
        financial_update_count = fu_q.scalar() or 0
        ...
        collection_rate = (
            structure.collection_rate_percent if structure else None
        )
        ...
        ctx = RunwayContext(
            ...
            runway_goal_months=(
                runway_setup.runway_goal_months if runway_setup else None
            ),
