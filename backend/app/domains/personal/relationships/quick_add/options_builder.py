"""Build Relationships quick-add options."""
...
def _hydrate_field_groups(metadata: dict[str, Any]) -> list[dict[str, Any]]:
    tabs = deepcopy(metadata.get("emotional_security_tabs") or [])
    for tab in tabs:
        for group in tab.get("field_groups") or []:
            ...
            if field_type not in ("single_select", "chip_grid", "slider", "multi_select", "panel_select"):
                continue
...
class RelationshipsQuickAddOptionsBuilder:
    def build(
        self,
        *,
        user_id: UUID,
        ...
        metadata = dict(RELATIONSHIPS_QUICK_ADD_METADATA)
        metadata["emotional_security_tabs"] = _hydrate_field_groups(RELATIONSHIPS_QUICK_ADD_METADATA)
        return {
            "moments": moment_options,
            "tabs": RELATIONSHIPS_QUICK_ADD_TABS,
            "categories": [],
            "accounts": accounts,
