...
_VISIBLE_STATUSES = frozenset({"PAUSED", "DRAFT", "ACTIVE", "SETUP", "COMPLETED"})
...
async def load_moment_inventories(
    ...
    list[MomentModel],
    list[MomentModel],
    dict[str, MomentModel],
]:
    """One list_by_context — return all, visible, active, latest_by_code."""
    all_moments = await MomentRepository(session).list_by_context(
        user_id, PERSONAL_CONTEXT
    )
...
async def sync_module_states(
    ...
    *,
    ...
    if visible_moments is None:
        _, visible_moments, _, _ = await load_moment_inventories(session, user_id)
