"""Relationships template activity handler."""
...
class RelationshipsActivityHandler:
    ...
    async def list_activity(
        ...
        *,
        ...
        items = await load_activity_list(
            ...
            user_id,
            moment.id,
            moment_type_code=code if code == _RS else _RS,
            limit=limit,
        )
        return {
    ...
    async def get_activity(self, user_id: UUID, event_id: str) -> dict[str, Any]:
        return await build_activity_detail(
            self.session, user_id, event_id, moment_type_code=_RS
        )
