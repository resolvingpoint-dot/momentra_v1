"""Future Building template activity handler."""
...
class FutureBuildingActivityHandler:
    ...
    async def list_activity(
        ...
        *,
        ...
        items = await load_activity_list(
            ...
            user_id,
            moment.id,
            moment_type_code=code if code == _FB else _FB,
            limit=limit,
        )
        return {
    ...
    async def get_activity(self, user_id: UUID, event_id: str) -> dict[str, Any]:
        return await build_activity_detail(
            self.session, user_id, event_id, moment_type_code=_FB
        )
    ...
    async def patch_activity(
        ...
        merged = dict(body)
        fb = body.get("future_building")
        if isinstance(fb, dict):
            nested = merged.get("future_building") or {}
            if not isinstance(nested, dict):
                nested = {}
            merged["future_building"] = {**nested, **fb}
