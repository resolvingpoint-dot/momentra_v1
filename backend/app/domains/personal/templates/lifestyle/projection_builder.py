"""Load Lifestyle projection context in a single pass."""
...
@dataclass
class LifestyleProjectionContext:
    user_id: UUID
    moment: MomentModel | None
    profile: PersonalLifestyleProfile | None = None
    ...
    signals: LifestyleSignals | None = None
...
class LifestyleProjectionBuilder:
    @staticmethod
    async def build(
        ...
        if moment is None:
            return LifestyleProjectionContext(user_id=user_id, moment=None)
        ...
        runtime_result = await session.execute(
            ...
            .where(
                ...
                PersonalRuntimeSnapshots.moment_type_code == _LS,
            )
        ...
        metrics_result = await session.execute(
            ...
            .where(
                ...
                PersonalMetricSnapshots.moment_type_code == _LS,
            )
        ...
        wellbeing_count = _count("WELLBEING")
        discovery_count = _count("DISCOVERY")
        expression_count = _count("EXPRESSION") + _count("CREATIVE")
        expense_count = _count("LIFESTYLE_EXPENSE")
        adjust_count = _count("ADJUST") + _count("LIFESTYLE_ADJUST")
        ...
        return LifestyleProjectionContext(
            user_id=user_id,
            moment=moment,
            profile=profile,
            runtime=runtime,
