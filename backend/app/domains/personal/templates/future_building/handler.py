"""Future Building template projection handler."""
...
class FutureBuildingTemplateHandler(BaseTemplateHandler):
    ...
    async def _build_context(
        ...
        moment = (
            ...
            if moment_id
            else self._moment_for_type(visible, _FB)
        )
        return await FutureBuildingProjectionBuilder.build(session, user_id, moment)

    async def moments(self, session: AsyncSession, user_id: UUID) -> dict[str, Any]:
        ctx = await self._build_context(session, user_id)
        base = build_future_building_moments(ctx, accounts_summary=_EMPTY_ACCOUNTS)
    ...
    async def pulse(self, session: AsyncSession, user_id: UUID) -> dict[str, Any]:
        ...
        if ctx.moment is None or ctx.moment.status != "ACTIVE":
            return {"moment_type_code": _FB, "status": "EMPTY", "pulse": None}
        block = build_future_building_pulse(ctx)
        cached = await ProjectionService(session).get_cached_context(user_id)
        return {
    ...
    async def memory(self, session: AsyncSession, user_id: UUID) -> dict[str, Any]:
        ctx = await self._build_context(session, user_id)
        cached = await ProjectionService(session).get_cached_context(user_id)
        envelope = ProjectionService(session).envelope(cached)
        if ctx.moment is None:
            return {
                **envelope,
                "moment_type_code": _FB,
        ...
        projection = build_future_building_memory(ctx) if status == "ACTIVE" else None
        cached = await ProjectionService(session).get_cached_context(user_id)
        return {
            **ProjectionService(session).envelope(cached),
            "moment_type_code": _FB,
