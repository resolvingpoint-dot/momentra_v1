"""Time-series helpers for Relationships pulse and moments projections."""
...
_TRUST_TYPES = frozenset({"SUPPORT", "CONNECTION"})
_CONNECTION_TYPES = frozenset({"CONNECTION", "SHARED_EXPERIENCE", "RELATIONSHIP_INVESTMENT"})
...
def build_trends_30d(timeline: list[PersonalActivityTimeline]) -> dict[str, list[dict[str, Any]]]:
    return {
        "trust": _counts_to_trend_points(daily_counts(timeline, _TRUST_TYPES)),
        "connection": _counts_to_trend_points(daily_counts(timeline, _CONNECTION_TYPES)),
    }
    ...
    *,
    ...
    for _label, month_start in month_starts:
        ...
        series.append(
            {
                "category_id": "spend",
                "category_name": "Relationship Spend",
...
def money_journey_highlights(
    ...
    for ev in money_events:
        ...
        cat = (ev.category_code or "relationships").replace("_", " ").title()
        by_cat[cat] += minor
